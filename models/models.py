...
def create_modules(module_defs, img_size, cfg):
    ...
    Parameters:
        module_defs: module definitions for parse yolo *.cfg file
        img_size: the size of image
        cfg: the file of yolo *.cfg
    ...
    for i, mdef in enumerate(module_defs):
        ...
        # 如果是可变形卷积
        ...
        elif mdef['type'] == 'carafe':
            modules = CARAFEPack(channels=mdef['filters'], scale_factor=mdef['stride'])

        elif mdef['type'] == 'route':  # nn.Sequential() placeholder for 'route' layer
            layers = mdef['layers']
            # 将layers里面所在层的输出通道数加起来
            filters = sum([output_filters[l + 1 if l > 0 else l] for l in layers])
            # 如果layers里面小于0，则加上当前层数为实际层数，在routs中一次性追加后面的内容
            routs.extend([i + l if l < 0 else l for l in layers])
            ...
            try:
                ...
                # modules.no：输出数量（class数量+5）， modules.na：anchor数量
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers, stride):
        ...
        self.no = nc + 5  # number of outputs (85 for coco)
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            if name in ['WeightedFeatureFusion', 'FeatureConcat', 'FeatureConcat2', 'FeatureConcat3', 'FeatureConcat_l',
                        'ScaleChannel', 'ScaleSpatial']:  # sum, concat
                if verbose:     # 如果为true的话，就会输出很多东西，verbose是冗长的意思
...
def get_yolo_layers(model):
    return [i for i, m in enumerate(model.module_list) if
            m.__class__.__name__ in ['YOLOLayer', 'JDELayer']]
